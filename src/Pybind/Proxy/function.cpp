...
std::string Function::getPybind() const {
  ...
  if (std::none_of(m_arguments.begin(), m_arguments.end(), [](const auto& arg) {
...
std::string Function::getArgumentTypes(bool withNames) const {
  ...
  std::transform(m_arguments.begin(),
                 ...
                 [=](auto const& argument) {
                   return withNames ? argument.typeName + " " + argument.name :
                                      argument.typeName;
                 });
...
std::string Function::getSignature() const {
  return fmt::format(
      ...
      fmt::arg("namespace",
               Pybind::Helpers::removeSubString(m_fullyQualifiedName, m_name)),
