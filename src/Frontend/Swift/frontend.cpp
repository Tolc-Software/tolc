...
createModule(IR::Namespace const& rootNamespace,
  ...
  if (auto maybeObjcFile =
          ::Objc::Builders::buildModuleFile(rootNamespace, "_" + moduleName)) {
    auto& objcFile = maybeObjcFile.value();

    out.push_back(
        std::make_pair(objcFile.getObjcHeaderFile(), objcFile.getObjcHeader()));
    out.push_back(
        std::make_pair(objcFile.getObjcSourceFile(), objcFile.getObjcSource()));
