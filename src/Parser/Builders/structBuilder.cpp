...
void addStructToVariant(std::optional<IRProxy::AccessModifier> modifier,
  ...
  if (auto ns = std::get_if<IR::Namespace*>(&v)) {
    ...
    if (auto structData =
            Helpers::getStructDataBasedOnAccess(**irStruct, modifier.value())) {
...
IR::Struct createStruct(IRProxy::Struct& s) {
  ...
  newStruct.m_hasImplicitDefaultConstructor = s.m_hasImplicitDefaultConstructor;
  return newStruct;
}
      ...
      for (auto& variable : variables.value()) {
        ...
        switch (variable.m_modifier) {
          ...
          case AccessModifier::Protected:
            s.m_protected.m_variables.push_back(variable.m_variable);
            break;
        }
      }
    }
