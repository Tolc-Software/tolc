...
void adjustConstructorName(IRProxy::Function& f, IRProxy::Struct const& s) {
  ...
  // Adjust the fully qualified name
  f.m_fullyQualifiedName = s.m_fullyQualifiedName + "::" + constructorFunction;
}
}    // namespace
...
bool ParserVisitor::VisitClassTemplateSpecializationDecl(
  ...
  auto policy = clang::PrintingPolicy(classDecl->getASTContext().getLangOpts());
  ...
  if (auto classTemplate =
    ...
    for (auto const& memberVariable : classTemplate->fields()) {
      auto memberType = getTemplatedType(helper, memberVariable->getType());
      if (auto maybeField = Builders::buildField(memberVariable, memberType)) {
        ...
        switch (access) {
          ...
          case AccessModifier::Protected:
            parsedStruct.m_protected.m_variables.push_back(variable);
            break;
        }
      } else {
    ...
    for (auto const& method : classTemplate->methods()) {
      if (auto parsedFunc =
              Visitor::Helpers::parseFunction(method, [&helper](auto type) {
