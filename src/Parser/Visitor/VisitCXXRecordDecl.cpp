...
bool ParserVisitor::VisitCXXRecordDecl(clang::CXXRecordDecl* classDecl) {
  ...
  parsedStruct.m_documentation = Visitor::Helpers::getDocumentation(classDecl);
  ...
  parsedStruct.m_modifier = Builders::convertToIRAccess(classDecl->getAccess());
  ...
  for (auto const& base : classDecl->bases()) {
    if (auto maybeAccess = ::Helpers::getStructDataBasedOnAccess(
      ...
      maybeAccess->m_inherited.push_back(base.getType().getAsString(policy));
    }
  }
