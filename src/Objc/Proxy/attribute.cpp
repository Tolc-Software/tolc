...
std::string Attribute::getObjcSource() const {
  ...
  auto call = ObjcSwift::Helpers::wrapInFunction(callAccess + m_name,
                                                 m_type.m_conversions.m_toObjc);
  out += fmt::format(R"(
...
std::string Attribute::getObjcHeader() const {
  ...
  if (m_isStandalone) {
    declaration =
        Objc::wrapInInterface(m_objcClass,
...
{declaration}
      ...
      fmt::arg("documentation", Objc::getDocumentationString(m_documentation)),
