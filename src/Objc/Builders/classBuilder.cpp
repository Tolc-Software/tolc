...
Objc::Proxy::Class buildClass(IR::Struct const& cppClass, Objc::Cache& cache) {
  Objc::Proxy::Class objcClass(Objc::getClassName(cppClass, cache.m_moduleName),
                               cppClass.m_representation);

  cache.m_cppToObjcClassNames[cppClass.m_representation] = objcClass.getName();
  ...
  for (auto const& function : cppClass.m_public.m_functions) {
    bool isOverloaded = overloadedFunctions.find(function.m_representation) !=
