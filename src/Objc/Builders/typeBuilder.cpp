...
Objc::Proxy::Type buildType(IR::Type const& type, Objc::Cache& cache) {
  ...
  if (auto baseType = std::get_if<IR::Type::Value>(&type.m_type)) {
    ...
    t.m_nullability = getNullable(baseType->m_base, type.m_numPointers != 0);
    t.m_name = [name]() {
      return name;
    };
    t.m_conversions =
    ...
    t.m_nullability = type.m_numPointers != 0 ? Proxy::Nullability::Nullable :
  ...
  } else if (auto container = std::get_if<IR::Type::Container>(&type.m_type)) {
    ...
    t.m_conversions =
        Objc::Conversions::getContainerTypeConversions(type, *container, cache);
    ...
    t.m_name = [cppClass = userDefined->m_representation, &cache]() {
      ...
      spdlog::error("Couldn't find the Objective-C class name of C++ class {}",
                    cppClass);
      return std::string();
    };
