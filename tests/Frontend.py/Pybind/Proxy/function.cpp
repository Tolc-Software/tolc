...
TEST_CASE("Static global function", "[function]") {
  ...
  REQUIRE(TestUtil::contains(pybindCode, R"(def_static("f", &MyNamespace::f)"));
}
...
TEST_CASE("Function with arguments", "[function]") {
  ...
  for (auto const& arg : args) {
    REQUIRE(
        TestUtil::contains(pybindCode, fmt::format(R"(py::arg("{}"))", arg)));
  }
}
...
TEST_CASE("Return value policy", "[function]") {
  ...
  for (auto const& policy : policies) {
    ...
    auto pybindCode = f.getPybind();
    auto containsString =
        fmt::format(R"(, py::{})", Pybind::returnValuePolicyToString(policy));
